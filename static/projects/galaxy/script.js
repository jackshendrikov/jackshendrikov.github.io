...
function vortex() {
  if (tick % 2 === 0 && params && params.vortex !== 0) {
    for (let i = 0; i < particles.geometry.vertices.length; i += 1) {
...
function spiral(params) {
  function lcg(value) {
    ...
    let multiplier =
      arguments.length > 2 && arguments[2] !== undefined
        ...
        : 1103515245;
    let increment =
      arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 12345;
  ...
  return {
    toArray: function toArray() {
      ...
      let points = [];
      let value = 0;
      for (let arm = 0; arm < params.arms; arm += 1) {
        for (let stop = 0; stop < params.stops; stop += 1) {
...
function onChange(params) {
  ...
  for (let i = 0; i < points.length; i += 1) {
    ...
    let pow = Math.max(
      0,
      (params.radius * 0.8 - distance) / (params.radius * 0.8)
    );
