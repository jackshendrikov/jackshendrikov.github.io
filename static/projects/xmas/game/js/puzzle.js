class Puzzle {
  ...
  solve() {
    ...
    for (let tileX = 0; tileX < Config.GRID_WIDTH_IN_TILES; tileX += 1) {
      for (let tileY = 0; tileY < Config.GRID_HEIGHT_IN_TILES; tileY += 1) {
        const center = this.grid_.getTileCenter(tileX, tileY);
        if (
          this.world_.anyPathBlockingObstacleInRegion(
            center.x,
            center.y,
  ...
  constructPath_(results, tile) {
    ...
    while (currentResult != null) {
      if (
        ...
        currentResult.targetCount === currentResult.parentResult.targetCount
      ) {
