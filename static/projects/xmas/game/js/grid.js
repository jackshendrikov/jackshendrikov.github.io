class Grid {
  ...
  reset(startY, endY, targetTile) {
    ...
    const fireplaceWidth =
      Config.WORLD_FIREPLACE_WIDTH_PX * Config.WORLD_FIREPLACE_SCALE;
    const fireplaceHeight =
      Config.WORLD_FIREPLACE_HEIGHT_PX * Config.WORLD_FIREPLACE_SCALE;
  ...
  renderRug_() {
    ...
    // Render the sides.
    for (tileX = 1; tileX < Config.GRID_WIDTH_IN_TILES - 1; tileX += 1) {
    ...
    for (tileY = 1; tileY < Config.GRID_HEIGHT_IN_TILES - 1; tileY += 1) {
  ...
  getTileCenter(tileX, tileY) {
    if (
      tileX < 0 ||
      tileX >= Config.GRID_WIDTH_IN_TILES ||
      tileY < 0 ||
      tileY >= Config.GRID_HEIGHT_IN_TILES
    ) {
  ...
  isInGrid(x, y) {
    return (
      x >= Config.GRID_TOP_LEFT_X &&
      x <=
        Config.GRID_TOP_LEFT_X +
          Config.GRID_TILE_SIZE_PX * Config.GRID_WIDTH_IN_TILES &&
      y >= Config.GRID_TOP_LEFT_Y &&
      y <=
        Config.GRID_TOP_LEFT_Y +
          Config.GRID_TILE_SIZE_PX * Config.GRID_HEIGHT_IN_TILES
    );
  }
}
