...
function addCircles() {
  ...
  for (let z = -fov; z < fov; z += 5) {
    ...
    for (let i = 0, l = coordinates.length; i < l; i += 1) {
...
function render() {
  ...
  for (let i = 0, l = circleHolder.length; i < l; i += 1) {
    ...
    for (let j = 0, k = circleObj.segmentsOutside.length; j < k; j += 1) {
      ...
      if (circleSegmentOutside.active === true) {
        ...
        circleSegmentOutsideangle =
          circleSegmentOutside.index *
            (MATHPI2 / circleSegmentOutside.segments) +
          time;
